...
void SpindleLaser::init() {
    ...
    #if ENABLED(DEBUG_BF_SPINDLE) 
      SERIAL_ECHOPGM("Spindle Init: set to off ");
      SERIAL_ECHO(!SPINDLE_LASER_ACTIVE_STATE);
      SERIAL_EOL(); 
    #endif

  #endif
  #if ENABLED(SPINDLE_CHANGE_DIR)
    #if ENABLED(ENABLE_BF_SPINDLE)
      // Motor A
      OUT_WRITE(SPINDLE_DIR_A1_PIN, !SPINDLE_INVERT_DIR);                   // Init rotation to clockwise (M3)
      OUT_WRITE(SPINDLE_DIR_A2_PIN, SPINDLE_INVERT_DIR);                   // Init rotation to clockwise (M3)
     
      // Motor B
      OUT_WRITE(SPINDLE_DIR_B1_PIN, !SPINDLE_INVERT_DIR);                   // Init rotation to clockwise (M3)
      OUT_WRITE(SPINDLE_DIR_B2_PIN, SPINDLE_INVERT_DIR);                   // Init rotation to clockwise (M3)
    #else 
    ...
    hal.set_pwm_frequency(pin_t(SPINDLE_LASER_PWM_PIN), SPINDLE_LASER_FREQUENCY);
      ...
      #if ENABLED(ENABLE_BF_SPINDLE)
        // Motor A
        SET_PWM(SPINDLE_PWM_A_PIN);
        hal.set_pwm_duty(pin_t(SPINDLE_PWM_A_PIN), SPINDLE_LASER_PWM_OFF); // Set to lowest speed
        
        // Motor B
        SET_PWM(SPINDLE_PWM_B_PIN);
        hal.set_pwm_duty(pin_t(SPINDLE_PWM_B_PIN), SPINDLE_LASER_PWM_OFF); // Set to lowest speed
    #else 
  ...
  void SpindleLaser::_set_ocr(const uint8_t ocr) {
    #if ENABLED(ENABLE_BF_SPINDLE)
        // Motor A
        hal.set_pwm_duty(pin_t(SPINDLE_PWM_A_PIN), ocr ^ SPINDLE_LASER_PWM_OFF);
        // Motor B
        hal.set_pwm_duty(pin_t(SPINDLE_PWM_B_PIN), ocr ^ SPINDLE_LASER_PWM_OFF);
        SERIAL_ECHOPGM("Spindle PWM Set to: ");
        SERIAL_ECHO(ocr);
        SERIAL_ECHOPGM(" out of 255: ");
        SERIAL_EOL(); 
    #else
      #if ENABLED(HAL_CAN_SET_PWM_FREQ) && SPINDLE_LASER_FREQUENCY
...
void SpindleLaser::apply_power(const uint8_t opwr) {
  #if ENABLED(DEBUG_BF_SPINDLE) 
    SERIAL_ECHOPGM("Spindle apply_power: ");
    SERIAL_ECHO(opwr);
    SERIAL_EOL(); 
  #endif
  if (enabled() || opwr == 0) {                                  // 0 check allows us to disable where no ENA pin exists
    ...
    // Handle PWM driven or just simple on/off
      ...
      // TODO remove (mrivera)
      #if ENABLED(DEBUG_BF_SPINDLE) 
        SERIAL_ECHOPGM("apply_power: turn spindle on ");
        SERIAL_ECHO(enabled() ? SPINDLE_LASER_ACTIVE_STATE : !SPINDLE_LASER_ACTIVE_STATE);
        SERIAL_EOL(); 
      #endif
  ...
  void SpindleLaser::set_reverse(const bool reverse) {
    ...
    if (TERN0(SPINDLE_STOP_ON_DIR_CHANGE, enabled()) && READ(SPINDLE_DIR_PIN) != dir_state) disable();
     ...
     #if ENABLED(ENABLE_BF_SPINDLE)
      // Motor A
      WRITE(SPINDLE_DIR_A1_PIN, !dir_state);                   // Init rotation to clockwise (M3)
      WRITE(SPINDLE_DIR_A2_PIN, dir_state);                   // Init rotation to clockwise (M3)
      
      
      // Motor B
      WRITE(SPINDLE_DIR_B1_PIN, !dir_state);                   // Init rotation to clockwise (M3)
      WRITE(SPINDLE_DIR_B2_PIN, dir_state);                   // Init rotation to clockwise (M3)
    
      #if ENABLED(DEBUG_BF_SPINDLE) 
        SERIAL_ECHOPGM("Spindle Set Reverse: ");
        SERIAL_ECHOPGM("A1: ");
        SERIAL_ECHO(!dir_state);
        SERIAL_ECHOPGM(" A2: ");
        SERIAL_ECHO(dir_state);
        SERIAL_EOL(); 
      #endif
